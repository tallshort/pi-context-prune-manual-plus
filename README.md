...
| `/pruner now` | Flush pending tool calls immediately (works in all modes) in a focusable centered progress overlay showing up to 16 batch rows. Press `Esc` to stop scheduling new batches; already-running batches finish and are retained. The overlay closes automatically when processing ends. |
...
- Pending batches with 600 or fewer raw result characters bypass the summarizer; larger batches are summarized together, with automatic paths using one call per batch in parallel and `/pruner now` using up to eight concurrent calls with live per-batch updates. Press `Esc` to stop scheduling more manual batches; already-running calls finish and completed results are retained.
...
flushPending()
  ...
  └─► summarize batches         parallel by default; `/pruner now` uses bounded concurrency (up to 8) with per-row progress
