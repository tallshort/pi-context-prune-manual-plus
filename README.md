...
The package has not been published to [npmjs.org](https://www.npmjs.com/) yet. After the first release, install the stable, versioned package with:
...
Once installed, the extension is auto-loaded every time you run `pi`. Re-run the command to upgrade to a newer npm release.
...
```bash
# Install globally (all projects)
pi install git:github.com/tallshort/pi-context-prune-manual-plus

# Or install for the current project only
pi install -l git:github.com/tallshort/pi-context-prune-manual-plus
```
> **Note:** Until the first npm release, install from GitHub. The `main` branch may contain unreleased or experimental changes.

### Try without installing

```bash
# Load the latest from git for this session only (no install)
pi -e git:github.com/tallshort/pi-context-prune-manual-plus
```
### From source (development)

```bash
git clone https://github.com/tallshort/pi-context-prune-manual-plus
cd pi-context-prune-manual-plus
pi -e .
```
...
| `minRawCharsThreshold` | Non-negative integer raw-character count; `0` disables skipping | `600` |
| `manualPruneConcurrency` | Integer simultaneous `/pruner now` summary calls, from `1` to `16` | `8` |
...
- `minRawCharsThreshold: 600` skips batches at or below 600 raw characters (about 150 tokens). Set it to `0` to summarize every batch.
